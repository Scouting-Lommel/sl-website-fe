...
const PageHead = ({ pageMeta, noIndex, pageTitle, slug, path, structuredData }) => {
  ...
  return (
    <Head>
      {/* PageMeta & SEO */}
      <title>{title}</title>
      <meta name="description" content={pageMeta?.pageDescription || general?.siteDescription} />
      <link rel="canonical" href={getUrl(locale, defaultLocale, path, slug)} />

      {/* Robots */}
      {(pageMeta?.noIndex || noIndex) && <meta name="robots" content="noindex,nofollow" />}
      ...
      />
      <meta property="og:image" content={general?.image?.data?.attributes?.url} />

      {/* Article Metadata */}
      <meta property="article:publisher" content="https://www.facebook.com/ScoutingLommel" />
      ...
      <meta
        ...
        content={pageMeta?.pageDescription || general?.siteDescription}
      />
      <meta name="twitter:image" content={general?.image?.data?.attributes?.url} />
      ...
      <link rel="mask-icon" href="/assets/head/safari-pinned-tab.svg" color="#eeb531" />
