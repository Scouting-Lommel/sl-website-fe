    ...
    return (
        <div>
             <li className="mb-10 ml-4">
                ...
                <time className="mb-1 text-sm font-normal leading-none text-gray-400 ">{parseDateTime(activity.attributes.startTime, activity.attributes.endTime)}</time>
                <div className="text-lg font-semibold text-gray-900 ">{activity.attributes.Title}</div>
                <p className="mb-4 text-base font-normal text-gray-500 ">{activity.attributes.description}</p>
            </li>
        </div>
    )
}
