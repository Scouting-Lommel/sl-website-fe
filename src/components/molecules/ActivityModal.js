...
import { deleteActivity } from '../../strapi/queries'
...
export default function ActivityModal({takName, takID, activities}) {
  ...
  const [removeActFunc, { loading, error }] = useMutation(deleteActivity, {
    variables: {
      actID: 0,
      },
    onCompleted(fin){
            ...
            {activities.map(act => {
                  ...
                  <div className="flex flex-row">
                    ...
                    <button onClick={() => {
                      ...
                      if(confirm(`are you sure you want to delete ${act.attributes.Title}?`)){
                        removeActFunc({
                          variables: {
                            actID: act.id,
                          },
