...
export default async function handler(req: Request, res: Response): Promise<unknown> {
  try {
    ...
    for(const dbDate of bookings.data){
      databaseCalendarData.push({id: dbDate.id, StartDate: dbDate.attributes.start, EndDate: dbDate.attributes.end});
    }

    console.warn(databaseCalendarData.length)
    console.warn(googleCalendarData.length)
