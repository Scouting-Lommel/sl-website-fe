...
export default function user({fin, general}) {
    if(fin.leaders.data.length == 0) return(<Layout generalData={general}></Layout>)
    const leader = fin.leaders.data[0].attributes;
  return (
    <Layout generalData={general} title={leader.FirstName + " " + leader.LastName} url="" noIndex={true}>
        <div className="py-5 flex min-h-[500px]">
            <div className="max-w-xl">
                <h1 className="text-5xl font-bold py-2">{leader.FirstName + " " + leader.LastName}</h1>
                    ...
                    <h1 className="text-2xl font-bold py-2">Rollen binnen de tak:</h1>
                        {
                            leader.GroupRoles.data.map((item, i) => {
                                return <h1 className="text-2xl font-bold py-2 pl-2" key={"role"+i}>{"- " + item.attributes.Name}</h1>
                            })
                        }
                    </>
                }
            ...
            <div className="flex flex-col justify-center flex-auto px-24 py-4">
                <div className="flex justify-center relative w-full h-full">
                    {leader.Image && leader.Image.data && leader.Image.data.attributes && <ComponentImage src={leader.Image.data.attributes.url}/>}
                </div>
            </div>
        </div>
    </Layout>
